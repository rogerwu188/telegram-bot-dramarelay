...
-- 在 users 表添加最后提交时间字段
ALTER TABLE users ADD COLUMN IF NOT EXISTS last_submission_time TIMESTAMP;

-- 在 user_tasks 表添加验证状态字段
ALTER TABLE user_tasks ADD COLUMN IF NOT EXISTS link_verified BOOLEAN DEFAULT FALSE;
ALTER TABLE user_tasks ADD COLUMN IF NOT EXISTS verification_time TIMESTAMP;
